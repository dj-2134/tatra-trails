...
function drawRoute(hike) {
  ...
  const casing = L.geoJSON(hike.geometry, { style: { className: "trail-casing", weight: 9, opacity: 1 } });
  const line = L.geoJSON(hike.geometry, { style: { className: `trail trail--${hike.status}`, weight: 5, opacity: 1 } });
